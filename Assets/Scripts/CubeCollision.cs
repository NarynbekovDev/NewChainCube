...
public class CubeCollision : MonoBehaviour {
   ...
   private void OnCollisionEnter (Collision collision) {
    ...
    public void OnCubesMerged(Cube cube)
    {
        // Добавляем очки при объединении
        int points = CalculatePointsForMerge(cube);
        ScoreManager.Instance.AddScore(points);
    }

    private int CalculatePointsForMerge(Cube cube)
    {
        // Например, за каждый объединённый куб даём 10 очков
        return cube.CubeNumber;
    }
}
